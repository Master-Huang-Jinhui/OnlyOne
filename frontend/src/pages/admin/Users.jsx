...
const emptyForm = { username: '', password: '', name: '', role: 'user', enabled: true }
...
export default function Users() {
  ...
  const load = () => api.getUsers().then(data => setUsers(Array.isArray(data) ? data : [])).catch(() => {})
  ...
  const save = async () => {
    ...
    try {
      ...
      else { await api.createUser(data); toast('创建成功') }
  ...
  const remove = async (id) => {
    ...
    await api.deleteUser(id); toast('已删除'); load()
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">管理系统用户和权限</p>
        </div>
      ...
      <Card>
        <Table columns={[
          { header: '账号', render: u => <div><p className="font-medium text-gray-800">{u.username}</p><p className="text-xs text-gray-400">{u.name || '-'}</p></div> },
          { header: '角色', render: u => <Badge variant={u.role === 'admin' ? 'primary' : 'default'}>{u.role === 'admin' ? '管理员' : '普通用户'}</Badge> },
          { header: '状态', render: u => <Badge variant={u.enabled ? 'success' : 'default'}>{u.enabled ? '启用' : '禁用'}</Badge> },
          { header: '创建时间', render: u => <span className="text-xs text-gray-400">{u.created_at}</span> }
      ...
      <Dialog open={dialog} onClose={() => setDialog(false)} title={editing ? '编辑用户' : '添加用户'}
        ...
        <div className="space-y-4">
          ...
          <Input label={editing ? '新密码（留空不修改）' : '密码 *'} type="password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} />
          <Input label="姓名" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />
          <Select label="角色" value={form.role} onChange={e => setForm({ ...form, role: e.target.value })} options={[{ value: 'user', label: '普通用户' }, { value: 'admin', label: '管理员' }]} />
