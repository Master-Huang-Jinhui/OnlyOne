    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Select value={dateRange} onChange={e => { setDateRange(e.target.value); setStartDate(''); setEndDate('') }}
            options={[{ value: '7', label: t('reports.range7', '最近7天') }, { value: '14', label: t('reports.range14', '最近14天') }, { value: '30', label: t('reports.range30', '最近30天') }, { value: '90', label: t('reports.range90', '最近90天') }]} />
      ...
      <div className="grid grid-cols-4 gap-4">
        <Card className="p-4 bg-gradient-to-br from-blue-500 to-blue-600 text-white">
          <p className="text-sm opacity-80">{t('reports.totalOrders', '总订单数')}</p>
          <p className="text-3xl font-bold mt-1">{overview.totalOrders}</p>
          <p className="text-xs opacity-75 mt-1">{t('reports.todayPrefix', '今日')} {overview.todayOrders} {t('reports.ordersUnit', '单')}</p>
        </Card>
        <Card className="p-4 bg-gradient-to-br from-green-500 to-green-600 text-white">
          <p className="text-sm opacity-80">{t('reports.totalRevenue', '总营收')}</p>
          <p className="text-3xl font-bold mt-1">${parseFloat(overview.totalRevenue || 0).toFixed(2)}</p>
          <p className="text-xs opacity-75 mt-1">{t('reports.todayPrefix', '今日')} ${parseFloat(overview.todayRevenue || 0).toFixed(2)}</p>
        </Card>
        <Card className="p-4 bg-gradient-to-br from-purple-500 to-purple-600 text-white">
          <p className="text-sm opacity-80">{t('reports.avgOrder', '平均客单价')}</p>
          <p className="text-3xl font-bold mt-1">${parseFloat(overview.avgOrder || 0).toFixed(2)}</p>
        </Card>
        <Card className="p-4 bg-gradient-to-br from-orange-500 to-orange-600 text-white">
          <p className="text-sm opacity-80">{t('reports.totalQty', '总销量（份）')}</p>
      ...
      <Card>
        <div className="px-5 py-3 border-b">
          <h3 className="font-bold text-gray-700">{t('reports.salesTrend', '📊 销售趋势')}</h3>
        </div>
        <div className="p-5">
          {trend.length === 0 ? (
            <Empty text={t('reports.noData', '暂无数据')} icon="📊" />
          ) : (
              ...
              {trend.map((item, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  ...
                  <div
                    ...
                    style={{ height: `${(item.revenue / maxTrendRevenue) * 140}px`, minHeight: '4px' }}
                    title={`${item.date}: ${item.orders}${t('reports.ordersUnit', '单')}, $${item.revenue.toFixed(2)}`}
                  />
      ...
      <div className="grid grid-cols-2 gap-6">
        <Card>
          <div className="px-5 py-3 border-b">
            <h3 className="font-bold text-gray-700">{t('reports.categorySales', '🍔 品类销售分析')}</h3>
          </div>
            ...
            {categorySales.length === 0 ? (
              ...
              <div key={i}>
                <div className="flex justify-between text-sm mb-1">
                  <span className="font-medium text-gray-700">{language === 'en' ? (c.category_name_en || c.category_name) : c.category_name}</span>
        ...
        <Card>
          <div className="px-5 py-3 border-b">
          <h3 className="font-bold text-gray-700">{t('reports.orderType', '🍽️ 订单类型分布')}</h3>
          </div>
          <div className="p-5">
            {diningTypeSales.length === 0 ? (
              <Empty text={t('reports.noData', '暂无数据')} icon="🍽️" />
            ) : (
                  ...
                  <div key={i} className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between text-sm">
                        <span className="font-medium text-gray-700">{language === 'en' ? (d.label_en || d.label || d.dining_type) : (d.label || d.dining_type)}</span>
                        <span className="text-gray-500">{d.orders}{t('reports.ordersUnit', '单')} · ${parseFloat(d.revenue).toFixed(2)}</span>
                      </div>
      ...
      <Card>
        <div className="px-5 py-3 border-b">
          <h3 className="font-bold text-gray-700">{t('reports.hourlyAnalysis', '⏰ 时段分析（按小时订单量）')}</h3>
        </div>
        <div className="p-5">
              ...
              {hourlySales.map((h, i) => (
                <div key={i} className="flex-1 flex flex-col items-center">
                  <div
                    ...
                    title={`${h.hour}:00 - ${h.orders}${t('reports.ordersUnit', '单')}, $${h.revenue.toFixed(2)}`}
                  />
      ...
      <Card>
        <div className="px-5 py-3 border-b">
          <h3 className="font-bold text-gray-700">{t('reports.topProducts', '🏆 TOP 10 热销商品')}</h3>
        </div>
        <div className="p-5">
            ...
            <div className="space-y-2">
              {topProducts.map((p, i) => (
                <div key={i} className="flex items-center gap-4 p-2 hover:bg-gray-50 rounded-lg">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between text-sm mb-1">
                      <span className="font-medium text-gray-700">{language === 'en' ? (p.name_en || p.name) : p.name}</span>
                      <span className="text-gray-500">{p.quantity}{t('reports.servingsUnit', '份')} · ${parseFloat(p.revenue).toFixed(2)}</span>
                    </div>
