...
export default function Dashboard() {
  ...
  const statusMap = {
    pending: { label: t('order.pending', '待处理'), variant: 'warning', next: 'preparing', nextLabel: t('order.startPreparing', '开始制作') },
    preparing: { label: t('order.preparing', '制作中'), variant: 'primary', next: 'ready', nextLabel: t('order.markReady', '制作完成') },
    ready: { label: t('order.ready', '待取餐'), variant: 'primary', next: 'completed', nextLabel: t('dashboard.confirmPickup', '确认取餐') },
    ...
    api.getOrders({ status: 'pending', page, page_size: pendingPageSize, sort_by: 'created_at', sort_order: 'desc', start_date: todayStart(), end_date: nowStr() })
      .then(data => {
        ...
        if (!isFirstLoad.current && total > lastPendingCount.current) {
          playOrderSound()
          vibrate()
          toast(`${t('dashboard.newOrderPrefix', '🔔 有新订单！当前')} ${total} ${t('dashboard.newOrderSuffix', '个待处理')}`, 'success')
        }
  ...
  const addMemo = async () => {
    ...
    try {
      ...
      toast(t('common.addSuccess', '添加成功'))
      setMemoDialog(false)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-gray-800">{t('admin.dashboard', '仪表盘')}</h2>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard title={t('dashboard.pendingOrders', '待处理订单')} value={stats.pending_count || 0} icon="⏳" color="yellow" />
      ...
      <Card className="overflow-hidden hover:shadow-lg transition-shadow cursor-pointer" onClick={() => navigate('/admin/stats/product')}>
        <div className="px-5 py-4 border-b flex items-center justify-between bg-gradient-to-r from-red-50 to-orange-50">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-gray-800">{t('dashboard.topProducts', '菜品热销 TOP5')}</h3>
          </div>
          <span className="text-xs text-gray-400">{t('dashboard.clickViewStats', '点击查看完整统计 →')}</span>
        </div>
        <div className="p-4">
          {topProducts.length === 0 ? (
            <p className="text-sm text-gray-400 text-center py-4">{t('dashboard.noSalesData', '暂无销售数据')}</p>
          ) : (
              ...
              {topProducts.map((p, i) => (
                <div key={p.product_id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50">
                  ...
                  <div className="flex items-center gap-4 text-xs">
                    <span className="text-gray-500">{p.order_count} {t('dashboard.ordersUnit', '单')}</span>
                    <span className="text-primary-600 font-bold">{p.total_sold} {t('dashboard.servingsUnit', '份')}</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <Card>
        <div className="px-5 py-4 border-b flex items-center justify-between">
          ...
          <Button size="sm" onClick={() => setMemoDialog(true)}>+ {t('common.add', '添加')}</Button>
        </div>
        <CardContent>
          {memos.length === 0 ? (
            <Empty text={t('dashboard.noMemos', '暂无备忘')} icon="📝" />
          ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant={m.priority === 'high' ? 'danger' : m.type === 'important' ? 'warning' : 'default'}>
                        {m.priority === 'high' ? t('dashboard.highPriority', '高优') : m.type === 'important' ? t('dashboard.important', '重点') : t('dashboard.memo', '备忘')}
                      </Badge>
                    </div>
      ...
      <Card>
        <div className="px-5 py-4 border-b flex items-center justify-between">
          <div className="flex items-center gap-2">
            <h3 className="font-semibold text-gray-800">{t('dashboard.pendingOrders', '今日待处理订单')}</h3>
            {pendingTotal > 0 && <Badge variant="warning">{pendingTotal}</Badge>}
          </div>
          <Link to="/admin/orders"><Button variant="ghost" size="sm">{t('dashboard.allOrders', '全部订单 →')}</Button></Link>
        </div>
        <CardContent>
          {pendingOrders.length === 0 ? (
            <Empty text={t('dashboard.noPendingOrders', '今日暂无待处理订单')} icon="✅" />
          ) : (
            <div className="space-y-2">
              {pendingOrders.map(o => (
                <button
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <Badge variant="warning">{t('order.pending', '待处理')}</Badge>
                    <div className="min-w-0">
                      <p className="font-mono text-sm font-medium text-gray-800">{o.order_no}</p>
                      <p className="text-xs text-gray-400 truncate">
                        {o.items?.slice(0, 3).map(it => it.name).join('、')}{o.items?.length > 3 ? ` ${t('dashboard.andMore', '等')}${o.items.length}${t('common.itemsUnit', '件')}` : ''}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 flex-shrink-0">
                    <div className="text-right">
                      ...
                      <p className="text-xs text-gray-400">{o.customer_name || t('dining.dinein', '堂吃')}</p>
                    </div>
      ...
      <Card>
        <div className="px-5 py-4 border-b flex items-center justify-between">
          ...
          <Link to="/admin/platforms"><Button variant="ghost" size="sm">{t('dashboard.manage', '管理')} →</Button></Link>
        </div>
        <CardContent>
          {platforms.length === 0 ? (
            <Empty text={t('dashboard.noPlatforms', '暂无平台，去添加吧')} icon="🛵" />
          ) : (
                ...
                <div key={p.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">{p.phone || t('dashboard.noPhone', '无电话')}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge variant={p.enabled ? 'success' : 'default'}>{p.enabled ? t('common.enabled', '启用') : t('common.disabled', '停用')}</Badge>
                    {p.url && <a href={`/go?platform=${p.id}`} target="_blank" rel="noreferrer" className="text-primary-500 text-sm hover:underline">{t('common.jump', '跳转')}</a>}
                  </div>
                </div>
              ))}
            </div>
