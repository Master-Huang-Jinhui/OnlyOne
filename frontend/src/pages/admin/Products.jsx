...
export default function Products() {
  ...
  useEffect(() => {
    api.getGoods({}).then(data => { setGoodsList(Array.isArray(data) ? data : []) }).catch(() => {})
  }, [])
  ...
  const addIngredient = () => { setIngredients(prev => [...prev, { goods_id: '', goods_name: '', quantity: 1, unit: '个' }]) }
  ...
  const updateIngredient = (index, field, value) => {
    setIngredients(prev => {
      ...
      if (field === 'goods_id') {
        ...
        if (goods) { next[index].goods_name = goods.name; next[index].unit = goods.unit || '个' }
      }
      return next
    })
  }
  ...
  const toggleSelectAll = (products) => {
    ...
    setSelectedIds(prev => { const next = new Set(prev); if (allSelected) products.forEach(p => next.delete(p.id)); else products.forEach(p => next.add(p.id)); return next })
  }
  ...
  const downloadTemplate = () => {
    ...
    fetch('/api/products/export/template', { method: 'POST', headers: { 'Authorization': `Bearer ${token}` } })
      .then(res => res.blob()).then(blob => { const url = window.URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'product_import_template.xlsx'; a.click(); window.URL.revokeObjectURL(url) })
  }

  const exportProducts = () => {
    const token = localStorage.getItem('token')
    fetch('/api/products/export', { method: 'POST', headers: { 'Authorization': `Bearer ${token}` } })
      .then(res => res.blob()).then(blob => { const url = window.URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'products_export.xlsx'; a.click(); window.URL.revokeObjectURL(url) })
  }
  ...
  const handleImportFile = async (e, confirm = false) => {
    ...
    try {
      ...
      if (data.needConfirm) {
        ...
        if (await confirm({ title: t('products.duplicateTitle', '发现重复商品'), message: `${t('products.duplicateCountPrefix', '发现')} ${data.duplicateCount} ${t('products.duplicateCountSuffix', '个重复商品：')}\n\n${dupList}\n\n${t('products.duplicateMessagePrefix', '是否跳过重复项，继续导入其余')} ${data.totalRows - data.duplicateCount} ${t('products.duplicateMessageSuffix', '条？')}`, variant: 'warning', confirmText: t('products.skipAndContinue', '跳过并继续'), cancelText: t('products.cancelImport', '取消导入') })) { handleImportFile(e, true); return }
  ...
  const load = () => {
    Promise.all([api.getAllCategories(), api.getAllProducts()]).then(([cats, prods]) => {
      ...
      const withProducts = categoryList.map(c => ({ ...c, products: productList.filter(p => p.category_id === c.id) }))
      const uncategorized = productList.filter(p => !p.category_id)
      if (uncategorized.length > 0) { withProducts.push({ id: 0, name: t('products.uncategorized', '未分类'), name_en: 'Uncategorized', enabled: 1, sort_order: 999, products: uncategorized }) }
  ...
  const saveProduct = async () => {
    ...
    try {
      const payload = { name: data.name.trim(), name_en: data.name_en || '', category_id: data.category_id || null, price: parseFloat(data.price), description: data.description || '', description_en: data.description_en || '', image: data.image || '', available: data.available ? 1 : 0, is_recommend: data.is_recommend ? 1 : 0, sort_order: data.sort_order || 0 }
      let productId = data.id
      if (mode === 'add') { const result = await api.createProduct(payload); productId = result.id; toast(t('products.productAdded', '商品已添加')) }
      else { await api.updateProduct(data.id, payload); toast(t('products.productUpdated', '商品已更新')) }
      if (productId && ingredients.length > 0) {
        const validIngredients = ingredients.filter(i => i.goods_id && i.quantity > 0)
        if (validIngredients.length > 0) { await api.saveProductIngredients(productId, validIngredients) }
      }
  ...
  const confirmMove = async () => {
    ...
    try {
      ...
      setMoveDialog(null); clearSelection(); load()
    } catch (e) { toast(e.message, 'error') }
  }
  ...
  const productColumns = [
    { header: '', render: p => (<input type="checkbox" checked={selectedIds.has(p.id)} onChange={() => toggleSelect(p.id)} className="w-4 h-4 cursor-pointer" />) },
    { header: t('products.product', '商品'), render: p => (<div className="flex items-center gap-3"><div className="w-12 h-12 bg-gray-100 rounded-lg flex items-center justify-center text-xl overflow-hidden flex-shrink-0">{p.image ? <img src={p.image} alt="" className="w-full h-full object-cover" /> : '🍜'}</div><div className="min-w-0"><p className="font-medium text-gray-800 truncate">{p.name}</p><p className="text-xs text-gray-400 truncate">{p.name_en || '-'}</p></div></div>) },
  ...
  const filteredCategories = kw ? categories.map(c => ({ ...c, products: (c.products || []).filter(p => (p.name || '').toLowerCase().includes(kw) || (p.name_en || '').toLowerCase().includes(kw) || (p.description || '').toLowerCase().includes(kw) || (p.description_en || '').toLowerCase().includes(kw)) })).filter(c => c.products.length > 0) : categories
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            <input type="text" value={searchKeyword} onChange={e => setSearchKeyword(e.target.value)} placeholder={t('products.searchPlaceholder', '搜索商品名称/描述...')} className="w-64 pl-9 pr-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent" />
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">🔍</span>
            {searchKeyword && (<button onClick={() => setSearchKeyword('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 text-sm">✕</button>)}
          </div>
      ...
      {displayCategories.length === 0 ? (
        ...
        <Card key={cat.id} className="overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 bg-gray-50 border-b">
            <div className="flex items-center gap-3">
              <button onClick={() => setExpanded(prev => ({ ...prev, [cat.id]: !prev[cat.id] }))} className="text-gray-500 hover:text-gray-700 w-6 text-center">{expanded[cat.id] ? '▼' : '▶'}</button>
              ...
              {expanded[cat.id] && cat.products?.length > 0 && (<label className="flex items-center gap-1 cursor-pointer text-xs text-gray-500"><input type="checkbox" checked={cat.products.every(p => selectedIds.has(p.id))} onChange={() => toggleSelectAll(cat.products)} className="w-3.5 h-3.5" />{t('common.selectAll', '全选')}</label>)}
            </div>
            <div className="flex items-center gap-2">
              {selectedIds.size > 0 && (<Button size="sm" variant="outline" className="border-indigo-300 text-indigo-600 hover:bg-indigo-50" onClick={openBatchMove}>{t('products.batchMovePrefix', '批量移动 (')}{selectedIds.size})</Button>)}
              {cat.id !== 0 && (<><Button size="sm" variant="outline" onClick={() => toggleCategory(cat)}>{cat.enabled ? t('common.disable', '禁用') : t('common.enable', '启用')}</Button><Button size="sm" variant="outline" onClick={() => setCatDialog({ mode: 'edit', data: { ...cat } })}>{t('products.editCategory', '编辑分类')}</Button></>)}
          ...
          {expanded[cat.id] && (
            <div className="p-5">
              {(!cat.products || cat.products.length === 0) ? (<Empty text={t('products.noProductsInCategory', '该分类下暂无商品，点击右上角 + 商品 添加')} icon="🍜" />) : (
                <Table columns={productColumns} data={cat.products} actions={p => (
                  <div className="flex items-center gap-3">
      ...
      <Dialog open={!!catDialog} onClose={() => setCatDialog(null)} title={catDialog?.mode === 'add' ? t('products.addCategory', '新增分类') : t('products.editCategory', '编辑分类')} width="max-w-sm">
        {catDialog && (
          <div className="space-y-4">
            ...
            <label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={catDialog.data.enabled} onChange={e => setCatDialog({ ...catDialog, data: { ...catDialog.data, enabled: e.target.checked } })} className="w-4 h-4" /><span className="text-sm text-gray-700">{t('products.enableCategoryHint', '启用该分类（禁用后前台不显示）')}</span></label>
            <div className="flex gap-2 pt-2"><Button variant="outline" className="flex-1" onClick={() => setCatDialog(null)}>{t('common.cancel', '取消')}</Button><Button className="flex-1" onClick={saveCategory}>{t('common.save', '保存')}</Button></div>
          </div>
        )}
      </Dialog>
        ...
        {productDialog && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              <Select label={t('products.category', '分类')} value={productDialog.data.category_id || ''} onChange={e => setProductDialog({ ...productDialog, data: { ...productDialog.data, category_id: e.target.value } })} options={[{ value: '', label: t('products.uncategorized', '未分类') }, ...categories.filter(c => c.id !== 0).map(c => ({ value: c.id, label: c.name }))]} />
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input type="text" value={productDialog.data.image} onChange={e => setProductDialog({ ...productDialog, data: { ...productDialog.data, image: e.target.value } })} placeholder={t('products.imageUrlPlaceholder', 'https://... or /uploads/images/xxx.jpg')} className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent" />
              ...
              {productDialog.data.image && (<div className="mt-2"><img src={productDialog.data.image} alt={t('products.preview', '预览')} className="w-24 h-24 object-cover rounded-lg border" /></div>)}
            </div>
            <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex gap-6">
              <label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={productDialog.data.available} onChange={e => setProductDialog({ ...productDialog, data: { ...productDialog.data, available: e.target.checked } })} className="w-4 h-4" /><span className="text-sm text-gray-700">{t('products.onSale', '在售')}</span></label>
              <label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={productDialog.data.is_recommend} onChange={e => setProductDialog({ ...productDialog, data: { ...productDialog.data, is_recommend: e.target.checked } })} className="w-4 h-4" /><span className="text-sm text-gray-700">{t('products.recommendProduct', '推荐商品')}</span></label>
            </div>
            <div className="border-t pt-4">
              ...
              {ingredients.length === 0 ? (<p className="text-xs text-gray-400 text-center py-3">{t('products.noIngredients', '暂无配料，点击上方按钮添加')}</p>) : (
                <div className="space-y-2">
                  {ingredients.map((ing, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                      <select value={ing.goods_id} onChange={e => updateIngredient(idx, 'goods_id', e.target.value)} className="flex-1 px-2 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"><option value="">{t('products.selectGoods', '选择货物...')}</option>{goodsList.map(g => (<option key={g.id} value={g.id}>{g.name} ({g.unit})</option>))}</select>
                      <input type="number" step="0.01" min="0" value={ing.quantity} onChange={e => updateIngredient(idx, 'quantity', parseFloat(e.target.value) || 0)} className="w-20 px-2 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder={t('products.quantity', '用量')} />
            ...
            <div className="flex gap-2 pt-2"><Button variant="outline" className="flex-1" onClick={() => setProductDialog(null)}>{t('common.cancel', '取消')}</Button><Button className="flex-1" onClick={saveProduct}>{t('common.save', '保存')}</Button></div>
          </div>
        )}
      </Dialog>
      ...
      <Dialog open={!!moveDialog} onClose={() => setMoveDialog(null)} title={moveDialog?.products.length > 1 ? `${t('products.batchMoveTitle', '批量移动')} ${moveDialog.products.length} ${t('products.batchMoveTitleSuffix', '个商品')}` : t('products.moveProductTitle', '移动商品到分类')} width="max-w-sm">
        {moveDialog && (
          <div className="space-y-4">
            {moveDialog.products.length === 1 ? (<p className="text-sm text-gray-600">{t('products.productLabel', '商品：')}<span className="font-medium text-gray-800">{moveDialog.products[0].name}</span></p>) : (<div className="text-sm text-gray-600 max-h-32 overflow-y-auto space-y-1">{moveDialog.products.map(p => <p key={p.id} className="truncate">• {p.name}</p>)}</div>)}
            <Select label={t('products.targetCategory', '目标分类')} value={moveDialog.targetCategoryId} onChange={e => setMoveDialog({ ...moveDialog, targetCategoryId: e.target.value })} options={[{ value: '', label: t('products.uncategorized', '未分类') }, ...categories.filter(c => c.id !== 0).map(c => ({ value: c.id, label: c.name }))]} />
            <div className="flex gap-2 pt-2"><Button variant="outline" className="flex-1" onClick={() => setMoveDialog(null)}>{t('common.cancel', '取消')}</Button><Button className="flex-1" onClick={confirmMove}>{t('products.confirmMove', '确认移动')}</Button></div>
          </div>
        )}
      </Dialog>
    </div>
