...
export default function FormRenderer() {
    ...
    for (const field of form.fields) {
      if (field.required) {
        ...
        if (field.type === 'checkbox') {
          if (!val || val.length === 0) {
            toast(t('forms.fillPrefix', '请填写「') + field.label + t('forms.fillSuffix', '」'), 'error')
            return false
          }
        } else if (!val || val === '') {
          toast(t('forms.fillPrefix', '请填写「') + field.label + t('forms.fillSuffix', '」'), 'error')
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true)
    try {
      await api.submitForm(id, values)
      setSubmitted(true)
      toast(t('forms.submitSuccess', '提交成功'))
    } catch (e) {
      toast(e.message, 'error')
    } finally {
      setSubmitting(false)
  ...
  if (!form) {
    return (
      <div className="flex items-center justify-center h-64">
        <Empty text={t('common.loading', '加载中...')} icon="⏳" />
      </div>
    )
  }
    ...
    return (
      <div className="max-w-2xl mx-auto">
        <Card className="p-12 text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-800 mb-2">{t('forms.submittedTitle', '提交成功')}</h2>
          <p className="text-gray-500 mb-6">{t('forms.submittedDesc', '感谢您的填写，我们已收到您的信息')}</p>
          <div className="flex justify-center gap-3">
            <Button variant="outline" onClick={resetForm}>{t('forms.fillAgain', '再填一份')}</Button>
          </div>
        </Card>
      </div>
    )
      ...
      case 'select':
        return (
          <Select
            ...
            value={values[field.label] || ''}
            onChange={e => handleChange(field.label, e.target.value)}
            options={[{ value: '', label: field.placeholder || t('forms.pleaseSelect', '请选择') }, ...options.map(o => ({ value: o, label: o }))]}
          />
        )
      case 'radio':
        return (
            ...
            <div className="flex flex-wrap gap-4">
              {options.map((opt, i) => (
                <label key={i} className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={(values[field.label] || []).includes(opt)}
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <Card className="p-6">
        {!form.fields || form.fields.length === 0 ? (
          <Empty text={t('forms.noFieldsPublic', '该表单暂无字段')} icon="📝" />
        ) : (
          <div className="space-y-5">
            {form.fields.map((field, idx) => renderField(field, idx))}
            <div className="pt-4 border-t">
              <Button onClick={handleSubmit} disabled={submitting} className="w-full">
                {submitting ? t('forms.submitting', '提交中...') : t('forms.submit', '提交')}
              </Button>
            </div>
          </div>
        )}
