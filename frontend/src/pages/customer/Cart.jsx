...
export default function Cart() {
  ...
  const saveNote = () => {
    if (noteDialog) updateNote(noteDialog.id, noteText)
    setNoteDialog(null)
  }
    ...
    <div className="min-h-screen bg-gray-50 pt-16">
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="space-y-4 mb-8">
          {items.map((item, idx) => (
            <div key={`${item.id}-${item.note}-${idx}`} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between mt-3">
                    <div className="flex items-center gap-2">
                      <button onClick={() => updateQuantity(item.id, item.quantity - 1, item.note)} className="w-7 h-7 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-600 flex items-center justify-center">−</button>
                      <span className="w-8 text-center font-medium">{item.quantity}</span>
                      <button onClick={() => updateQuantity(item.id, item.quantity + 1, item.note)} className="w-7 h-7 rounded-full bg-primary-100 hover:bg-primary-200 text-primary-600 flex items-center justify-center">+</button>
                    </div>
                    <div className="flex gap-2 text-sm">
                      <button onClick={() => openNoteDialog(item)} className="text-primary-600 hover:text-primary-700">{item.note ? '修改备注' : '加备注'}</button>
                      {item.note && <button onClick={() => clearNote(item.id)} className="text-gray-400 hover:text-gray-600">清除</button>}
                      <button onClick={() => removeItem(item.id, item.note)} className="text-red-400 hover:text-red-600">删除</button>
                    </div>
                  </div>
                </div>
              </div>
