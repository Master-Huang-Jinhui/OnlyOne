...
export function CartProvider({ children }) {
  ...
  // 拆分口味：从某行分出 splitQty 份，用新口味，剩下的保留原口味
  const addSplitFlavor = (cartId, splitQty, newNotes) => {
    setItems(prev => {
      const item = prev.find(i => i.cartId === cartId)
      if (!item) return prev
      ...
      return [
        ...prev.map(i => i.cartId === cartId ? { ...i, quantity: remaining } : i),
        newLine
      ]
    })
  }
  ...
  return (
    <CartContext.Provider value={{
      items, addItem, updateQuantity, updateNotes, addSplitFlavor, clearNotes, removeItem, clear,
