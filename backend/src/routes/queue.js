...
// 给queue_numbers表添加recall_count字段（兼容旧数据库）
try {
  const cols = db.prepare("PRAGMA table_info(queue_numbers)").all()
  if (!cols.find(c => c.name === 'recall_count')) {
    db.prepare("ALTER TABLE queue_numbers ADD COLUMN recall_count INTEGER DEFAULT 0").run()
  }
} catch (e) {}
...
router.post('/list', auth, (req, res) => {
  ...
  const queues = db.prepare(`SELECT * FROM queue_numbers ${where} ORDER BY 
    CASE status WHEN 'waiting' THEN 1 WHEN 'calling' THEN 0 ELSE 2 END,
    created_at ASC`).all(...params)
  res.json(queues)
})
...
router.post('/take', auth, (req, res) => {
  ...
  const result = db.prepare(`
    INSERT INTO queue_numbers (number, type, customer_name, note, people_count, status, created_at)
    VALUES (?, ?, ?, ?, ?, 'waiting', datetime('now','localtime'))
  `).run(number, type, customer_name, note, people_count)
...
// 再叫一次（当前叫号中的号码可以重复叫号）
router.post('/recall/:id', auth, (req, res) => {
  const queue = db.prepare('SELECT * FROM queue_numbers WHERE id = ?').get(req.params.id)
  if (!queue) return res.status(404).json({ error: '排队号不存在' })
  if (queue.status !== 'calling') return res.status(400).json({ error: '只有叫号中的号码才能再叫一次' })
  db.prepare("UPDATE queue_numbers SET called_at = datetime('now','localtime'), recall_count = COALESCE(recall_count, 0) + 1 WHERE id = ?").run(req.params.id)
  res.json({ message: `再次叫号 ${queue.number}`, recall_count: (queue.recall_count || 0) + 1 })
})
...
router.post('/stats', auth, (req, res) => {
  ...
  const avgWait = db.prepare(`
    SELECT AVG((julianday(called_at) - julianday(created_at)) * 24 * 60) as avg_minutes
    FROM queue_numbers 
    WHERE called_at IS NOT NULL AND date(created_at) = ?
  `).get(today).avg_minutes
