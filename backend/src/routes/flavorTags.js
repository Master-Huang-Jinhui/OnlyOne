...
// 获取所有启用的口味标签（前台用，只返回启用大类下的启用小类），支持按商品分类过滤
router.get('/', (req, res) => {
  ...
  if (product_category_id) {
    categories = categories.filter(cat => {
      if (!cat.category_ids) return false;
      try { const ids = JSON.parse(cat.category_ids); return ids.length > 0 && ids.includes(parseInt(product_category_id)); } catch (e) { return false; }
    });
  }
