...
// 给goods表添加min_stock字段（兼容旧数据库）
try {
  const cols = db.prepare("PRAGMA table_info(goods)").all();
  if (!cols.find(c => c.name === 'min_stock')) {
    db.prepare("ALTER TABLE goods ADD COLUMN min_stock REAL DEFAULT 0").run();
  }
} catch (e) {}
...
router.post('/goods', auth, managerAccess, (req, res) => {
  ...
  const result = db.prepare(`INSERT INTO goods (name, name_en, unit, current_stock, avg_price, supplier, category, note) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(
    name, name_en, unit, current_stock, avg_price, supplier, category, note
  );
...
router.post('/orders', auth, managerAccess, (req, res) => {
  ...
  const tx = db.transaction(() => {
    const result = db.prepare(`INSERT INTO purchase_orders (order_no, supplier, order_date, total_amount, delivery_fee, discount, payment_method, note) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(
      order_no, supplier, order_date, total_amount, delivery_fee, discount, payment_method, note
    );
    ...
    for (const item of items) {
      ...
      // 如果没有 goods_id，根据名称查找或创建
      if (!goodsId && item.goods_name) {
        const existing = db.prepare('SELECT id FROM goods WHERE name = ? LIMIT 1').get(item.goods_name);
        if (existing) {
          goodsId = existing.id;
        } else {
          const r = insertGoods.run(item.goods_name, item.unit || '个', qty, price, supplier);
          goodsId = r.lastInsertRowid;
        }
      }
      ...
      // 更新货物库存和均价
      if (goodsId) {
        updateGoods.run(qty, price, price, goodsId);
      }
    }
...
router.post('/orders/update/:id', auth, managerAccess, (req, res) => {
  ...
  const tx = db.transaction(() => {
    ...
    if (items) {
      for (const item of items) {
        total_amount += (parseFloat(item.quantity) || 0) * (parseFloat(item.unit_price) || 0);
      }
    }
    ...
    if (items) {
      ...
      for (const item of items) {
        const qty = parseFloat(item.quantity) || 0;
        const price = parseFloat(item.unit_price) || 0;
        insertItem.run(req.params.id, item.goods_id || null, item.goods_name, qty, item.unit || '个', price, qty * price);
      }
    }
  });
...
// ===== 商品配料管理 =====
router.post('/ingredients/list', auth, (req, res) => {
  const { product_id } = req.query;
  if (!product_id) return res.status(400).json({ error: 'product_id 必填' });
  const ingredients = db.prepare('SELECT * FROM product_ingredients WHERE product_id = ? ORDER BY id').all(product_id);
  res.json(ingredients);
});

router.post('/ingredients/save', auth, managerAccess, (req, res) => {
  const { product_id, ingredients = [] } = req.body;
  if (!product_id) return res.status(400).json({ error: 'product_id 必填' });

  const tx = db.transaction(() => {
    db.prepare('DELETE FROM product_ingredients WHERE product_id = ?').run(product_id);
    const insert = db.prepare('INSERT INTO product_ingredients (product_id, goods_id, goods_name, quantity, unit) VALUES (?, ?, ?, ?, ?)');
    for (const ing of ingredients) {
      if (!ing.goods_id || !ing.quantity) continue;
      const goods = db.prepare('SELECT name, unit FROM goods WHERE id = ?').get(ing.goods_id);
      if (!goods) continue;
      insert.run(product_id, ing.goods_id, goods.name, parseFloat(ing.quantity) || 0, ing.unit || goods.unit || '个');
    }
  });
  tx();
  res.json({ success: true });
...
// ===== 库存预警 =====
router.post('/alerts', auth, managerAccess, (req, res) => {
  const alerts = db.prepare('SELECT * FROM goods WHERE min_stock > 0 AND current_stock <= min_stock ORDER BY current_stock ASC').all();
  res.json(alerts);
});

// ===== 库存流水 =====
router.post('/transactions/list', auth, managerAccess, (req, res) => {
  const { goods_id, type, start_date, end_date, limit = 100 } = req.query;
  let sql = 'SELECT * FROM inventory_transactions WHERE 1=1';
  const params = [];
  if (goods_id) { sql += ' AND goods_id = ?'; params.push(goods_id); }
...
// ===== 手动库存调整 =====
router.post('/goods/adjust', auth, managerAccess, (req, res) => {
  const { goods_id, quantity, type = 'out', note = '' } = req.body;
  if (!goods_id || !quantity) return res.status(400).json({ error: 'goods_id 和 quantity 必填' });

  const goods = db.prepare('SELECT * FROM goods WHERE id = ?').get(goods_id);
  if (!goods) return res.status(404).json({ error: '货物不存在' });

  const qty = parseFloat(quantity) || 0;
  const tx = db.transaction(() => {
    if (type === 'out') {
      db.prepare('UPDATE goods SET current_stock = current_stock - ?, updated_at = datetime(\'now\',\'localtime\') WHERE id = ?').run(qty, goods_id);
    } else {
      db.prepare('UPDATE goods SET current_stock = current_stock + ?, updated_at = datetime(\'now\',\'localtime\') WHERE id = ?').run(qty, goods_id);
    }
    db.prepare('INSERT INTO inventory_transactions (goods_id, goods_name, type, quantity, unit, note) VALUES (?, ?, ?, ?, ?, ?)').run(
      goods_id, goods.name, type, qty, goods.unit, note
    );
  });
  tx();
  res.json({ success: true });
