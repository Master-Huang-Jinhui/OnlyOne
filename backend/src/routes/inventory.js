...
router.get('/goods', auth, managerAccess, (req, res) => {
...
router.get('/goods/:id', auth, managerAccess, (req, res) => {
...
router.put('/goods/:id', auth, managerAccess, (req, res) => {
...
router.delete('/goods/:id', auth, managerAccess, (req, res) => {
  db.prepare('DELETE FROM goods WHERE id = ?').run(req.params.id);
  res.json({ success: true });
});

router.get('/orders', auth, managerAccess, (req, res) => {
...
router.get('/orders/:id', auth, managerAccess, (req, res) => {
  ...
  const tx = db.transaction(() => {
    ...
    for (const item of items) {
      ...
      if (!goodsId && item.goods_name) {
        ...
        if (existing) goodsId = existing.id;
        else { const r = insertGoods.run(item.goods_name, item.unit || '个', qty, price, supplier); goodsId = r.lastInsertRowid; }
      }
      insertItem.run(orderId, goodsId, item.goods_name, qty, item.unit || '个', price, qty * price);
...
router.put('/orders/:id', auth, managerAccess, (req, res) => {
...
router.delete('/orders/:id', auth, managerAccess, (req, res) => {
  db.prepare('DELETE FROM purchase_order_items WHERE purchase_order_id = ?').run(req.params.id);
