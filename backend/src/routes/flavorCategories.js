...
const isApplicable = (flavorCat, productCategoryId) => { if (!productCategoryId) return true; const ids = parseCategoryIds(flavorCat.category_ids); return ids.length > 0 && ids.includes(parseInt(productCategoryId)); };
...
router.post('/list', (req, res) => {
  ...
  if (product_category_id) categories = categories.filter(cat => isApplicable(cat, product_category_id));
