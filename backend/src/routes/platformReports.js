...
function extractDataFromText(text) {
  ...
  if (fullText.includes('grubhub') || fullText.includes('seamless')) result.platform_name = 'Grubhub';
  else if (fullText.includes('uber eats') || fullText.includes('uber technologies') || fullText.includes('ubereats')) result.platform_name = 'Uber Eats';
  else if (fullText.includes('doordash') || fullText.includes('door dash')) result.platform_name = 'DoorDash';
  else if (fullText.includes('postmates')) result.platform_name = 'Postmates';
  else if (fullText.includes('chownow') || fullText.includes('chow now')) result.platform_name = 'ChowNow';
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!result.total_sales && /restaurant sales|gross sales|total sales|total revenue/i.test(lower)) {
      ...
      if (m && /\$/.test(line)) result.total_sales = parseFloat(m[1].replace(/,/g, ''));
      else { const amount = findNextAmount(i); if (amount) result.total_sales = amount; }
    }
    
    if (!result.order_count) {
      const m = line.match(/for\s+(\d+)\s+orders/i);
      if (m) result.order_count = parseInt(m[1]);
    ...
    if (!result.platform_fee && /grubhub order services|platform fee|total fees|service fees|commission/i.test(lower)) {
      let m = line.match(/\$\(([0-9,]+\.\d{2})\)/) || line.match(/\$?\(?([0-9,]+\.\d{2})\)?/);
      if (m && (/\$|\(/.test(line))) result.platform_fee = Math.abs(parseFloat(m[1].replace(/,/g, '')));
      else { const amount = findNextAmount(i); if (amount) result.platform_fee = Math.abs(amount); }
    }
    
    if (!result.net_revenue && /balance|net revenue|payout|total payout|net earnings|total payments to you/i.test(lower)) {
      let m = line.match(/\$\(([0-9,]+\.\d{2})\)/) || line.match(/\$?([0-9,]+\.\d{2})/);
      if (m && /\$/.test(line)) result.net_revenue = parseFloat(m[1].replace(/,/g, ''));
      else { const amount = findNextAmount(i); if (amount) result.net_revenue = amount; }
    }
...
function parseCSVReport(filePath, fileName) {
  ...
  if (fileName.toLowerCase().includes('doordash') || headers.some(h => h.toLowerCase().includes('doordash'))) result.platform_name = 'DoorDash';
  else if (fileName.toLowerCase().includes('ubereats') || fileName.toLowerCase().includes('uber eats')) result.platform_name = 'Uber Eats';
  else if (fileName.toLowerCase().includes('grubhub')) result.platform_name = 'Grubhub';
  ...
  let totalSales = 0, orderCount = 0, refundCount = 0, refundAmount = 0, totalNet = 0;
  for (const row of rows) {
    const rowType = typeCol ? String(row[typeCol]).toLowerCase() : '';
    if (netCol) { const val = parseFloat(String(row[netCol]).replace(/[$,]/g, '')); if (!isNaN(val)) totalNet += val; }
    if (rowType === 'order') {
      orderCount++;
      if (subtotalCol) { const val = parseFloat(String(row[subtotalCol]).replace(/[$,]/g, '')); if (!isNaN(val)) totalSales += val; }
    } else if (rowType.includes('error') || rowType.includes('refund') || rowType.includes('chargeback')) {
      refundCount++;
      if (netCol) { const val = parseFloat(String(row[netCol]).replace(/[$,]/g, '')); if (!isNaN(val) && val < 0) refundAmount += Math.abs(val); }
    }
  }
...
router.get('/', auth, (req, res) => {
  const reports = db.prepare(`SELECT r.*, p.name as platform_name FROM platform_reports r LEFT JOIN platforms p ON r.platform_id = p.id ORDER BY r.month DESC, r.created_at DESC`).all();
  res.json({ reports });
});
...
router.post('/upload', auth, managerAccess, upload.single('file'), async (req, res) => {
  ...
  if (ext === '.pdf') {
    try {
      ...
      if (!finalPlatformId && extracted.platform_name) {
        ...
        if (platform) { finalPlatformId = platform.id; extractedNote = '（自动识别平台）'; }
      }
      extractedNote = extractedNote || '（自动提取）';
    } catch (e) { console.error('PDF 解析失败:', e); }
  } else if (ext === '.csv') {
    try {
      ...
      extractedNote = extractedNote || '（自动提取）';
    } catch (e) { console.error('CSV 解析失败:', e); }
  }
  ...
  if (!force) {
    const existing = db.prepare('SELECT id FROM platform_reports WHERE platform_id = ? AND month = ?').get(finalPlatformId, finalMonth);
    if (existing) {
      return res.status(409).json({ error: '该平台这个月已经有报表了', duplicate: true, existing_id: existing.id });
    }
  }

  const result = db.prepare(`INSERT INTO platform_reports (platform_id, month, file_path, original_name, total_sales, order_count, refund_count, refund_amount, platform_fee, net_revenue, note) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
    finalPlatformId, finalMonth, `/uploads/reports/${path.basename(req.file.path)}`, req.file.originalname,
    finalTotalSales, finalOrderCount, finalRefundCount, finalRefundAmount, finalPlatformFee, finalNetRevenue,
