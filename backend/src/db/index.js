...
['commission_rate REAL DEFAULT 0',"payout_schedule TEXT DEFAULT 'weekly'","delivery_type TEXT DEFAULT 'platform'",'min_order REAL DEFAULT 0','delivery_radius REAL DEFAULT 0',"contact_person TEXT DEFAULT ''",'rating REAL DEFAULT 0',"launch_date TEXT DEFAULT ''"].forEach(col => { try { db.prepare(`ALTER TABLE platforms ADD COLUMN ${col}`).run(); } catch (e) {} });

// ========== 菜单 v7：完整奶茶饮品 + 小料 ==========
try {
  const flag = db.prepare("SELECT value FROM settings WHERE key = 'menu_reset_v7'").get();
  if (!flag) {
    console.log('[菜单] v7 重置：完整饮品单 + 小料...');
    ...
    [
      ...
      ['甜品','Dessert',2],
      ['饮品','Drinks',3],
    ].forEach(([n,en,s]) => { catIds[n] = insertCat.run(n, en, s).lastInsertRowid; });
    ...
    const drinks = [
      ...
      ['黑糖奥利奥冰淇淋','Brown Sugar Oreo W/ Ice Cream',7.95,'photo-1558857563-b39609b3b971'],
      // 脏奶
    ...
    s = 1; drinks.forEach(([n,en,p,pid]) => ins.run(n, en, catIds['饮品'], p, img(pid), s++));

    // 小料 Toppings -> flavor_categories + flavor_tags
    ...
    const toppings = [
      ...
      ['布丁','Pudding',0.75],
      ['珍珠','Tapioca',0.75],
      ['荔枝椰果','Lychee Jelly',0.75],
      ['芒果椰果','Mango Jelly',0.75],
      ['爆爆珠','Popping Ball',0.75],
      ['玛奇朵奶霜','Macchiato Cream',1.25],
      ['奶盖','Cheese Foam',1.25],
      ['奥利奥碎','Oreo Crumbs',1.25],
      ['冰淇淋','Ice Cream',1.50],
    ];
    const insTag = db.prepare('INSERT INTO flavor_tags (category_id, category, name, name_en, price, sort_order, enabled) VALUES (?, ?, ?, ?, ?, ?, 1)');
    toppings.forEach(([n,en,p], i) => insTag.run(fcId, '小料', n, en, p, i+1));

    db.prepare("INSERT INTO settings (key, value) VALUES ('menu_reset_v7', 'done')").run();
    console.log(`[菜单] v7完成：主食${staples.length} + 甜品${desserts.length} + 饮品${drinks.length} + 小料${toppings.length}`);
  }
} catch (e) {
  console.error('[菜单] v7重置失败:', e.message);
}
